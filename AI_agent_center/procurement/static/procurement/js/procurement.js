    ...
    const app = {
        ...
        attachEventListeners() {
            ...
            this.cancelBulkUploadBtn.addEventListener('click', () => {
                ...
                this.fileUploadInput.value = ''; this.fileUploadStatus.textContent = '';
            });
        ...
        async openDrillDownModal(requestId) {
            ...
            if (!response.ok) { alert('Could not fetch request details.'); return; }
        ...
        async handleAddNewRequest(e) { e.preventDefault(); const formData = { title: document.getElementById('product-name').value, quantity: document.getElementById('quantity').value, specs: document.getElementById('specs').value, source: 'Manual' }; const response = await fetch('/procurement/api/add-request/', { method: 'POST', headers: {'Content-Type': 'application/json', 'X-CSRFToken': this.csrfToken}, body: JSON.stringify(formData) }); if (response.ok) { this.addRequestModal.classList.add('hidden'); e.target.reset(); window.location.reload(); } else alert('Failed to add request.'); },
