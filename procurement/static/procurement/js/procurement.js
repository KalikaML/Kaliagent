...
document.addEventListener('DOMContentLoaded', () => {
    const app = {
        ...
        generateApprovalPanel(request) {
            ...
            if (suppliers.length > 0) {
                supplierListHtml = suppliers.map(s => {
                    const provenanceBadge = s.provenance === 'existing'
                        ? '<span class="ml-2 px-2 py-1 text-xs rounded bg-emerald-700 text-white">existing</span>'
                        : '<span class="ml-2 px-2 py-1 text-xs rounded bg-indigo-700 text-white">scraped</span>';
                    const sourceLink = s.source_link ? `<a href="${s.source_link}" target="_blank" class="text-sky-400 hover:underline">Source</a>` : '';
                    return `
            ...
            const rfqBody = (
                `Dear Supplier,\n\n` +
                `We are seeking a quotation for the item below. Kindly provide a complete response with the requested fields so we can compare fairly.\n\n` +
                `Product Details:\n` +
                `- Product: ${request.title}\n` +
                `- Quantity: ${request.quantity}\n` +
                `- Specifications: ${request.specs || 'As per standard'}\n\n` +
                `Required Quote Data (please fill every item):\n` +
                `Pricing:\n` +
                `- Unit Price (INR): [ ]\n` +
                `- Subtotal (INR, before taxes & freight): [ ]\n` +
                `- Tax Amount (INR, e.g., GST): [ ]\n` +
                `- Freight/Shipping Charges (INR): [ ]\n` +
                `- Grand Total (INR, after taxes & freight): [ ]\n` +
                `- Discount (% or INR, specify type): [ ]\n\n` +
                `Delivery:\n` +
                `- Lead Time (calendar days): [ ]\n\n` +
                `Payment Terms:\n` +
                `- Terms (e.g., On delivery / 50% advance / 100% advance): [ ]\n\n` +
                ...
                `- Reply to this email with the details above in the body, or attach a PDF/Excel with these fields clearly labeled.\n` +
                `- If multiple options exist (grades/specs), list each line item separately with its own unit price, lead time, and terms.\n` +
                `Acceptance Criteria:\n` +
                `- Complete pricing (unit, taxes, freight, total).\n` +
                `- Lead time aligned with our delivery needs.\n` +
                `- Payment terms compatible with our policies.\n` +
                `- Clear product identification (name/SKU/model).\n` +
                `- Validity and warranty stated.\n\n` +
                `Thank you,\nKalika Enterprises\n` +
                `Office Address: Plot No M-59, MIDC, Ahmednagar - 414 001 (M.S.) INDIA`
            );
        ...
        generateQuotesPanel(request) {
            ...
            const bestQuoteId = request.best_quote_id;
            const bestQuoteScore = request.best_quote_score;
            const bestBreakdown = request.best_quote_breakdown || {};
            ...
            if (quotes.length === 1) {
                ...
                <div class="bg-slate-900 p-6 rounded-lg h-full flex flex-col">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        ${(() => {
                            const productIdForFindMore = request.representative_request_id ? request.id : request.product_id;
                            return `<button id="find-more-suppliers-btn" data-product-id="${productIdForFindMore}" class="bg-sky-600 hover:bg-sky-500 text-white font-semibold py-2 px-3 rounded-lg text-sm">+ Find More Suppliers</button>`;
                        })()}
                    </div>
            ...
            // Sort by backend-computed score if present, else fallback to price
            quotes.sort((a, b) => {
                const as = (typeof a.score === 'number') ? a.score : (a.price ?? Infinity);
                const bs = (typeof b.score === 'number') ? b.score : (b.price ?? Infinity);
                return as - bs;
            });
            const quotesHtml = quotes.map((q) => {
                const isBest = bestQuoteId && q.id === bestQuoteId;
                const rowClass = isBest ? 'bg-slate-800/70' : '';
                const priceClass = isBest ? 'text-green-400' : 'text-slate-300';
                const bestQuoteBadge = isBest ? '<span class="ml-2 text-xs font-bold bg-green-500 text-white py-0.5 px-2 rounded-full">🏆 Best Quote</span>' : '';
                const scoreText = (typeof q.score === 'number') ? ` (score: ${q.score.toFixed(2)})` : '';
                return `<tr class="border-b border-slate-700 hover:bg-slate-800 ${rowClass}">
                    ...
                    <td class="p-3 text-right">
                        <span class="text-xs text-slate-400 mr-2">${scoreText}</span>
            ...
            return `
                <div class="bg-slate-900 p-6 rounded-lg h-full flex flex-col">
                    ...
                    <div class="mb-4">
                        <p class="text-sm text-slate-400">Quotes are ranked by composite score (lower is better).</p>
                        ${bestQuoteId ? `
                        <div class="mt-2 bg-slate-800 p-3 rounded-lg border border-slate-700">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-sm text-slate-400">Best Quote ID</p>
                                    <p class="font-bold">#${bestQuoteId}</p>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm text-slate-400">Composite Score</p>
                                    <p class="font-bold text-green-400">${(typeof bestQuoteScore === 'number') ? bestQuoteScore.toFixed(2) : 'N/A'}</p>
                                </div>
                            </div>
                            <div class="grid grid-cols-2 gap-3 mt-3 text-xs">
                                <div class="bg-slate-700/40 p-2 rounded"><span class="text-slate-400">Effective Cost:</span> <span class="font-semibold">₹${bestBreakdown.effective_cost !== undefined ? parseFloat(bestBreakdown.effective_cost).toFixed(2) : 'N/A'}</span></div>
                                <div class="bg-slate-700/40 p-2 rounded"><span class="text-slate-400">Lead Penalty:</span> <span class="font-semibold">${bestBreakdown.lead_penalty !== undefined ? bestBreakdown.lead_penalty.toFixed(2) : 'N/A'}</span></div>
                                <div class="bg-slate-700/40 p-2 rounded"><span class="text-slate-400">Terms Penalty:</span> <span class="font-semibold">${bestBreakdown.terms_penalty !== undefined ? bestBreakdown.terms_penalty.toFixed(2) : 'N/A'}</span></div>
                                <div class="bg-slate-700/40 p-2 rounded"><span class="text-slate-400">Missing Penalty:</span> <span class="font-semibold">${bestBreakdown.missing_penalty !== undefined ? bestBreakdown.missing_penalty.toFixed(2) : 'N/A'}</span></div>
                            </div>
                        </div>` : ''}
                    </div>
                    <div class="flex-grow overflow-y-auto">
                        <!-- Manual Quote Upload Form -->
                        <div class="mb-4 bg-slate-800 p-4 rounded-lg border border-slate-700">
                            <h4 class="font-bold mb-2">Add Manual Quote</h4>
                            <form id="manual-quote-form" class="grid grid-cols-2 lg:grid-cols-4 gap-3" enctype="multipart/form-data">
                                <input class="p-2 rounded bg-slate-700 text-sm" name="supplier_name" placeholder="Supplier Name" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="supplier_email" placeholder="Email (optional)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="supplier_phone" placeholder="Phone (optional)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="price" placeholder="Unit Price (₹)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="quantity" placeholder="Quantity" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="lead_time_days" placeholder="Lead Time (days)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="payment_terms" placeholder="Payment Terms" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="discount" placeholder="Discount (e.g., 5% or 500)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="subtotal" placeholder="Subtotal (₹)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="tax_amount" placeholder="Tax Amount (₹)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="freight_charges" placeholder="Freight (₹)" />
                                <input class="p-2 rounded bg-slate-700 text-sm" name="total_amount" placeholder="Grand Total (₹)" />
                                <input type="file" class="p-2 rounded bg-slate-700 text-sm col-span-2" name="attachment" />
                                <textarea class="p-2 rounded bg-slate-700 text-sm col-span-2" name="full_text" placeholder="Quote text or notes"></textarea>
                                <div class="col-span-full flex justify-end">
                                    <button id="submit-manual-quote" type="button" class="bg-green-600 hover:bg-green-500 text-white text-sm font-semibold py-2 px-3 rounded">Upload & Include</button>
                                </div>
                            </form>
                        </div>
             ...
             return `
                <div class="bg-slate-900 p-6 rounded-lg h-full flex flex-col">
                    ...
                    <div class="flex-grow overflow-y-auto bg-slate-800 p-4 rounded-md border border-slate-700">
                     ...
                     <div class="mt-4">
                         <a href="${quote.attachment_url}" target="_blank" class="text-sm text-sky-400 hover:underline">Download attached file</a>
                     </div>` : ''}
                </div>`;
        },
        ...
        attachModalEventListeners(request) {
            ...
            // Manual quote upload handler
            const form = document.getElementById('manual-quote-form');
            const submitBtn = document.getElementById('submit-manual-quote');
            if (form && submitBtn) {
                submitBtn.addEventListener('click', async () => {
                    const formData = new FormData(form);
                    try {
                        const resp = await fetch(`/procurement/api/upload-quote/${this.currentRequestData.representative_request_id || this.currentRequestData.id}/`, {
                            method: 'POST',
                            headers: { 'X-CSRFToken': this.csrfToken },
                            body: formData,
                        });
                        ...
                        if (!resp.ok || data.success === false) {
                            ...
                            const errList = Object.entries(errors).map(([k, v]) => `${k}: ${v}`).join(', ');
                            const msg = data.error || (errList || 'Upload failed');
                            alert(msg);
                            return;
                        }
                        // Refresh panel with latest quotes
                        this.currentRequestData = data;
                        const mainPanel = this.modalContentContainer.querySelector('.lg\\:col-span-2');
                        if (mainPanel) {
                            mainPanel.innerHTML = this.generateQuotesPanel(this.currentRequestData);
                        }
                        this.addLog('📥 Manual quote uploaded and included in comparison.');
                    } catch (e) {
                        alert('Upload failed');
                    }
                });
